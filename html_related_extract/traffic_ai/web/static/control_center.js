...
const TWIN_FRAME_INTERVAL_MS = 1000 / 30;
...
function renderBreakdown(breakdown) {
    ...
    const renderKey = JSON.stringify(entries.slice(0, 8));
    if (element.dataset.renderKey === renderKey) {
        return;
    }
    element.dataset.renderKey = renderKey;
...
function renderCorridors(corridors) {
    for (const name of ["north", "east", "south", "west"]) {
        ...
        const nextWidth = `${Math.max(0, Math.min(100, Number(corridor.pressure || 0)))}%`;
        if (bar) {
            if (bar.style.width !== nextWidth) {
...
function renderChart(containerId, entries, key, color, statId) {
    ...
    if (!Array.isArray(entries) || !entries.length) {
        const emptyMarkup = "<div class='chart-empty'>Waiting for live data.</div>";
    ...
    const renderKey = `${key}:${values.join(",")}`;
    if (container.dataset.renderKey === renderKey) {
        if (stat) {
            stat.textContent = `Latest ${values[values.length - 1].toFixed(1)} | Peak ${Math.max(...values).toFixed(1)}`;
        }
        return;
    }
...
function updateState(state) {
    ...
    renderChart("chart-vehicles", timeline, "vehicles", "#38bdf8", "chart-vehicles-stat");
    renderChart("chart-congestion", timeline, "congestion", "#f97316", "chart-congestion-stat");
    renderChart("chart-throughput", timeline, "throughput", "#2dd4bf", "chart-throughput-stat");
    renderChart("chart-mobility", timeline, "mobility", "#f472b6", "chart-mobility-stat");
}
...
function resizeTwinCanvas() {
    if (!twinRuntime.canvas) {
        return;
    }
    const rect = twinRuntime.canvas.getBoundingClientRect();
    const cssWidth = Math.max(1, Math.round(rect.width));
    const cssHeight = Math.max(1, Math.round(rect.height));
    const pixelRatio = Math.min(window.devicePixelRatio || 1, 1.5);

    if (
        twinRuntime.cssWidth === cssWidth &&
        twinRuntime.cssHeight === cssHeight &&
        twinRuntime.pixelRatio === pixelRatio
    ) {
        return;
    }
    ...
    twinRuntime.cssHeight = cssHeight;
    twinRuntime.pixelRatio = pixelRatio;
    twinRuntime.canvas.width = Math.round(cssWidth * pixelRatio);
    twinRuntime.canvas.height = Math.round(cssHeight * pixelRatio);
    if (twinRuntime.ctx) {
        twinRuntime.ctx.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0);
    }
}

function drawTwin() {
    if (!pageVisible) {
        twinRafId = requestAnimationFrame(drawTwin);
        return;
    }
    if (!twinRuntime.canvas) {
        twinRuntime.canvas = document.getElementById("twin-canvas");
        if (!twinRuntime.canvas) {
            twinRafId = requestAnimationFrame(drawTwin);
            return;
        }
        twinRuntime.ctx = twinRuntime.canvas.getContext("2d", { alpha: false, desynchronized: true });
        resizeTwinCanvas();
    }

    const now = performance.now();
    if (now - twinRuntime.lastFrameAt < TWIN_FRAME_INTERVAL_MS) {
        twinRafId = requestAnimationFrame(drawTwin);
        return;
    }
    twinRuntime.lastFrameAt = now;
    resizeTwinCanvas();

    const canvas = twinRuntime.canvas;
    const ctx = twinRuntime.ctx;
    if (!ctx) {
        twinRafId = requestAnimationFrame(drawTwin);
        return;
    }
    const width = twinRuntime.cssWidth || canvas.clientWidth || canvas.width;
    const height = twinRuntime.cssHeight || canvas.clientHeight || canvas.height;
