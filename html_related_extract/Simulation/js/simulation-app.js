        function showBootError(message) {
            ...
            if (bannerTitle) {
                bannerTitle.textContent = "Simulation startup issue";
            }
            if (bannerText) {
        ...
        window.addEventListener("error", event => {
            showBootError(`Simulation error: ${event.message || "Unknown script failure."}`);
        });

        const createSimulationRenderer = window.SmartTrafficSimulationRender?.createRenderer;
        if (typeof createSimulationRenderer !== "function") {
            showBootError("Renderer script did not load. Check js/simulation-render.js and reload the page.");
            throw new Error("Simulation renderer is unavailable.");
        }
        const canvas = document.getElementById("sim-canvas");
        if (!canvas) {
            showBootError("Simulation canvas was not found in the page.");
            throw new Error("Simulation canvas missing.");
        }
        const ctx = canvas.getContext("2d", { alpha: false, desynchronized: true });
        if (!ctx) {
            showBootError("This browser could not initialize a 2D canvas context.");
        ...
        function easedFrameBlend(baseBlend, dt = 16.6667) {
            const base = clamp(baseBlend, 0, 0.95);
            const frameScale = clamp(dt / 16.6667, 0.35, 2.25);
            return clamp(1 - Math.pow(1 - base, frameScale), 0, 0.95);
        }
        ...
        function motionSmoothingAlpha(dt, responseMs = 62) {
            const frameMs = clamp(Number(dt) || 16.6667, 4, 50);
            return clamp(1 - Math.exp(-frameMs / responseMs), 0.08, 0.62);
        }
        ...
        function isHeldAtStopLine(vehicle, bufferPx = 8) {
            if (!vehicle || vehicle.state !== "STRAIGHT") return false;
            const stopDistance = vehicle.getStopLinePos() - vehicle.getPos();
            const light = trafficLights.getLight(vehicle.direction);
            return (
                light !== LIGHT.GREEN &&
                vehicle.speed < 1.15 &&
                stopDistance > -bufferPx
            );
        }
        ...
        class DisciplineManager {
            ...
            shouldApplyRandomStop(vehicle, distToStop) {
                return this.isActive() && state.flowMode === "fast" && vehicle.canApplyRandomStop(distToStop);
            }
        ...
        class Vehicle {
            ...
            isTurnPathClear(turnKind, distToStop = Infinity) {
                ...
                for (const other of state.vehicles) {
                    if (other.id === this.id || other.state === "DONE") continue;
                    if (isHeldAtStopLine(other, 10)) continue;
            ...
            resetVisualState() {
                ...
                this.visualYieldOffset = this.yieldOffset;
            }

            syncVisualState(dt, force = false) {
                if (
                    force ||
                    !Number.isFinite(this.visualX) ||
                    !Number.isFinite(this.visualY) ||
                    !Number.isFinite(this.visualAngle)
                ) {
                    this.resetVisualState();
                    return;
                }

                const jumpDistance = Math.hypot(this.x - this.visualX, this.y - this.visualY);
                if (jumpDistance > Math.max(150, this.length * 4.5)) {
                    this.resetVisualState();
                    return;
                }

                const positionAlpha = motionSmoothingAlpha(dt, this.state === "TURNING" ? 48 : 58);
                const angleAlpha = motionSmoothingAlpha(dt, this.state === "TURNING" ? 38 : 52);
                this.visualX = lerp(this.visualX, this.x, positionAlpha);
                this.visualY = lerp(this.visualY, this.y, positionAlpha);
                this.visualAngle = smoothAngle(this.visualAngle, this.angle, angleAlpha);
                this.visualYieldOffset = lerp(
                    this.visualYieldOffset,
                    this.yieldOffset,
                    motionSmoothingAlpha(dt, 70)
                );
            }
            ...
            update(dt) {
                ...
                if (
                    ...
                    Math.random() < Math.min(0.02 * flow.randomStopRateScale, (dt / 22000) * flow.randomStopRateScale)
                ) {
                    this.randomStopTimerMs = 120 + Math.random() * 420;
                    this.randomStopCooldownMs = 4200 + Math.random() * 6400;
                }

                if (this.randomStopTimerMs > 0 && disciplineManager.shouldApplyRandomStop(this, distToStop)) {
                    targetSpeed = Math.min(
                        targetSpeed,
                        this.randomStopTimerMs > 220 ? freeFlow * 0.1 : freeFlow * 0.2
                    );
                }
                ...
                if (this.state === "TURNING") {
                    ...
                    const laneBlend = easedFrameBlend(disciplineManager.getLaneBlend(this), dt);
                ...
                const easing = easedFrameBlend(this.yielding ? 0.2 : 0.12, dt);
                this.yieldOffset += (this.yieldOffsetTarget - this.yieldOffset) * easing;
                this.syncVisualState(dt);
            }

            draw() {
                const drawX =
                    this.direction === DIR.NORTH || this.direction === DIR.SOUTH
                        ? this.visualX + this.visualYieldOffset
                        : this.visualX;
                const drawY =
                    this.direction === DIR.EAST || this.direction === DIR.WEST
                        ? this.visualY + this.visualYieldOffset
                        : this.visualY;
        ...
        function targetPixelRatio() {
            ...
            return state.performance.quality === "reduced" ? Math.min(1.2, dpr) : Math.min(1.75, dpr);
        }
        ...
        function resize() {
            ...
            state.performance.pixelRatio = pixelRatio;
        ...
        function updatePerformanceMode(frameCostMs) {
            state.performance.lastFrameCostMs = frameCostMs;
            if (frameCostMs > 26) {
                state.performance.slowFrames = Math.min(30, state.performance.slowFrames + 1);
            } else if (frameCostMs < 18) {
                state.performance.slowFrames = Math.max(0, state.performance.slowFrames - 1);
            }
            const nextQuality = state.performance.slowFrames >= 8 ? "reduced" : "normal";
            if (nextQuality !== state.performance.quality) {
                state.performance.quality = nextQuality;
                resize();
                return;
            }
            state.performance.quality = nextQuality;
        ...
        function dynamicLiveCap(congestion) {
            const baseCap = congestion >= 90 ? 72 : congestion >= 78 ? 86 : 108;
            const performanceTrim = state.performance.quality === "reduced" ? 18 : 0;
            return Math.max(56, baseCap - performanceTrim);
        }
        ...
        function canSpawnVehicle(candidate) {
            for (const vehicle of state.vehicles) {
                ...
                const sameApproach = vehicle.originDirection === candidate.originDirection;
                const minGap = sameLane
                    ...
                    : sameApproach
                        ? config.carLength * 2.8
                        : config.carLength * 1.9;
                if (distance < minGap) return false;
                if (sameApproach && distance < candidate.length * 2.6) return false;
            }
            return true;
        }
        ...
        function attemptSpawn(options = {}) {
            if (!options.emergencyType && state.metrics.liveVehicles >= dynamicLiveCap(state.metrics.congestionIndex)) {
                return null;
            }
        ...
        function resolveVehicleOverlaps() {
            ...
            const minBase = config.carLength * 0.74 * flow.junctionCautionScale * congestionScale;
            const passes = state.metrics.congestionIndex >= 82 ? 2 : 1;
            for (let pass = 0; pass < passes; pass += 1) {
                for (let i = 0; i < state.vehicles.length; i += 1) {
                    ...
                    for (let j = i + 1; j < state.vehicles.length; j += 1) {
                        ...
                        const sameLane = a.direction === b.direction && a.lane === b.lane;
                        const nearConflictZone =
                            a.isInJunctionZone(34) ||
                            b.isInJunctionZone(34) ||
                            a.state === "TURNING" ||
                            b.state === "TURNING";
                        if (!sameLane && !nearConflictZone) continue;
                        ...
                        const minSeparation = Math.max(
                            minBase,
                            ((a.length + b.length) * 0.42 + config.safeDistance * 0.2) * flow.gapScale
                        );
                        if (dist >= minSeparation) continue;

                        const overlap = minSeparation - dist;
                        const correction = Math.min(overlap, config.carLength * (sameLane ? 0.18 : 0.1));
                        const nx = dx / dist;
                        const ny = dy / dist;
                        ...
                        if (sameLane) {
                            ...
                            follower.speed = Math.min(follower.speed, leader.speed * 0.9);
                        } else {
                            a.speed *= 0.96;
                            b.speed *= 0.96;
                        }
        ...
        function updateDashboard() {
            ...
            setTextSafe(
                "status-note",
                `${state.network.controlNote} Ingress governor ${ingressScalePct}% (${trendLabel}). Motion smoothing active. Performance ${state.performance.quality}, frame ${state.performance.lastFrameCostMs.toFixed(1)} ms.`
            );
        ...
        function updateSimulation(dt) {
            ...
            const congestion = state.metrics.congestionIndex;
            const liveCap = dynamicLiveCap(congestion);
            ...
            const heavyFrame = state.performance.quality === "reduced" || state.metrics.liveVehicles > 82;
            if (!heavyFrame || state.performance.frameIndex % 2 === 0) {
        ...
        function loop(timestamp) {
            ...
            if (frameStart - state.performance.lastDashboardPaintAt >= 80) {
                updateDashboard();
                state.performance.lastDashboardPaintAt = frameStart;
            }
            updatePerformanceMode(performance.now() - frameStart);
